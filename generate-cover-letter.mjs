...
Usage:
  ...
  --skip-facts  Skip the CV fact-check gate (on-demand compositions already grounded in the source profile)
`);
  ...
  try {
    ...
    // cannot leave behind a misleading artifact. --skip-facts bypasses the
    // gate for on-demand compositions whose metrics were already grounded in
    // the source-of-truth prompt profile (worker fallback safety net).
    if (!args["skip-facts"]) {
